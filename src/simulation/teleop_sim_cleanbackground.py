...
class ServoTeleoperatorSim: 
    ...
    def __init__(self, scene: str, robot_uids: str, serial_port: str = '/dev/ttyUSB0'):
        ...
        self.ser = serial.Serial(self.SERIAL_PORT, self.BAUDRATE, timeout=0.1)  # 增加超时
        time.sleep(0.2)  # 等待串口稳定
        self.ser.reset_input_buffer()  # 清空初始缓冲区
        ...
        self.rate = 20.0  # 降低到20Hz，确保串口稳定
        
        # 软件限位：相对于零点的最大偏移角度（度）
        # 这样可以避免舵机撞到机械限位
        self.angle_limits = [
            (-135, 135),  # ID 0: 基座，可以全范围旋转
            (-90, 90),    # ID 1: 肩部，限制±90°
            (-90, 90),    # ID 2: 肘部，限制±90°
            (-90, 90),    # ID 3: 腕1，限制±90°
            (-90, 90),    # ID 4: 腕2，限制±90°
            (-90, 90),    # ID 5: 腕3，限制±90°
            (-135, 135),  # ID 6: 夹爪，可以全范围
        ]
        ...
        # Create simulation environment with clean background
        # 使用 Empty-v1 场景可以获得干净的背景
        self.env = gym.make(
            scene,
            robot_uids=robot_uids,
            render_mode="human",
            control_mode=self.control_mode,
            sensor_configs=dict(shader_pack="default"),  # 使用基础渲染
    ...
    def _clean_background(self):
        ...
        try:
            ...
            if scene is not None:
                # 尝试隐藏地面
                ground = getattr(self.env.unwrapped, "ground", None)
                if ground is not None:
                    try:
                        # 方法1: 隐藏地面渲染
                        ground.set_visibility(False)
                        print("[INFO] Ground hidden (set_visibility)")
                    except:
                        try:
                            # 方法2: 禁用地面
                            ground.disable()
                            print("[INFO] Ground disabled")
                        except:
                            pass
                ...
                try:
                    ...
                    print("[INFO] Ambient light set to dark")
                except:
                    pass
                    
            print("[INFO] Background cleaned - only robot arm is visible")
        except Exception as e:
            print(f"[INFO] Background cleanup note: {e}")
    ...
    def _setup_camera_pose(self):
        ...
        if self.robot_uids == "unitree_h1":
            camera_pose = sapien_utils.look_at([2.0, -2.0, 1.5], [0.0, 0.0, 0.5])
        elif self.robot_uids in ["x_fetch"]:
            camera_pose = sapien_utils.look_at([1.5, -2.0, 1.5], [0.0, 0.0, 0.5])
        else:
            # 标准机械臂视角
        ...
        if camera_viewer is not None:
            ...
            # 尝试设置背景颜色为纯色 (深色)
            try:
                camera_viewer.set_background_color([0.05, 0.05, 0.05, 1.0])  # 近黑色
                print("[INFO] Background color set to dark (near black)")
            except Exception as e:
                print(f"[INFO] Could not set background color: {e}")
    ...
    def _init_servos(self):
        ...
        time.sleep(0.1)  # 等待版本查询完成
        
        for i in range(7):
            self.send_command("#000PCSK!")
            time.sleep(0.05)  # 命令间延迟
            
            self.send_command(f'#{i:03d}PULK!')
            time.sleep(0.05)
            
            response = self.send_command(f'#{i:03d}PRAD!')
            time.sleep(0.05)  # 读取后延迟
            
            angle = self.pwm_to_angle(response.strip())
            if angle is None:
                print(f"[WARN] Servo {i} response incomplete: {response.strip()}, retrying...")
                time.sleep(0.1)
                response = self.send_command(f'#{i:03d}PRAD!')
                angle = self.pwm_to_angle(response.strip())
            
            self.zero_angles[i] = angle if angle is not None else 0.0
            print(f"[INFO] Servo {i} initial angle: {self.zero_angles[i]:.2f}°")
            
        print("[INFO] Servo initial angle calibration completed")

    def send_command(self, cmd: str, retries: int = 2) -> str:
        """Send serial command and read response with retry mechanism
        ...
        for attempt in range(retries):
            ...
            time.sleep(0.03)  # 30ms 延迟，平衡速度和稳定性
            response = self.ser.read_all()
            decoded = response.decode('ascii', errors='ignore') if response else ""
            
            # 检查响应是否完整（至少包含P和4位数字）
            if 'P' in decoded and len(decoded) >= 8:
                return decoded
    ...
    def pwm_to_angle(self, response_str: str, pwm_min: int = 500, 
        ...
        Args:
            ...
            pwm_min: PWM minimum value (500)
            pwm_max: PWM maximum value (2500)
            angle_range: Angle range in degrees (270°)
            
        Returns:
            Angle value, returns None if parsing fails or out of range
        """
        ...
        # 验证PWM值是否在有效范围内
        if pwm_val < pwm_min or pwm_val > pwm_max:
            print(f"[WARN] PWM value {pwm_val} out of range [{pwm_min}, {pwm_max}]")
            return None
        ...
        # 验证角度是否在有效范围内
        if angle < 0 or angle > angle_range:
            print(f"[WARN] Angle {angle:.2f}° out of range [0, {angle_range}]")
            return None
    ...
    def convert_pose_to_action(self, pose: list) -> np.ndarray: 
        ...
        elif self.robot_uids == "rm65b":  # RM65-B 6-axis collaborative robot
            # RM65-B 使用标准6轴配置，不需要夹爪
            pose_copy = pose.copy()
            pose_copy.pop(6)  # 移除第7个舵机（夹爪）
            action = np.array(pose_copy)
            # RM65-B 关节方向调整（根据实际测试调整）
            # action[1] = -action[1]  # 如需要可取消注释
    ...
    def teleop_sim_handler(self, action: np.ndarray, dwell: float = 0.01):
        ...
        try:
            ground = getattr(self.env.unwrapped, "ground", None)
            if ground is not None and hasattr(ground, 'set_visibility'):
                ground.set_visibility(False)
        except:
            pass
        ...
        while not self.stop_event.is_set():
            ...
            for i in range(num_joints):
                ...
                if angle is not None:
                    ...
                    # 检查是否超出软件限位
                    min_limit, max_limit = self.angle_limits[i]
                    if new_angle < min_limit or new_angle > max_limit:
                        print(f"[WARN] Servo {i} angle {new_angle:.1f}° exceeds limit [{min_limit}, {max_limit}]")
                        # 限制在安全范围内
                    ...
                    # 读取失败，使用上一次的值，不抛出异常
                    read_success = False
                    if i == 0:  # 只在第一个舵机失败时打印警告
                        print(f"[WARN] Servo read failed, using previous values")
                    break  # 跳过本次读取
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        default='so100',
        choices=['arx-x5', 'so100', 'xarm6_robotiq', 'panda', 'x_fetch', 'piper', 'widowx250s', 'rm65b', 'unitree_h1'],
        help='Select robot arm type to control'
    )
    parser.add_argument(
        '--scene', '-s', 
        type=str, 
        default='Empty-v1',  # 默认使用空场景
        help='Simulation scene name (use Empty-v1 for clean background)'
    )
    parser.add_argument(
        '--rate', 
        type=float, 
