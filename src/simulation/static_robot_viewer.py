...
class StaticRobotViewer:
    ...
    def _setup_robot_pose(self):
        ...
        try:
            ...
            elif self.robot_uids == "rm65b":
                # RM65-B robot arm
                if self.pose_name == "home":
                    # Set to home position
                    home_pose = np.array([0.0, -0.785, 1.57, 0.0, 0.785, 0.0])
                    agent.reset(home_pose)
                    print("RM65-B set to home pose")
                elif self.pose_name == "ready":
                    # Set to ready position
                    ready_pose = np.array([0.0, -1.047, 1.57, 0.0, 1.047, 0.0])
                    agent.reset(ready_pose)
                    print("RM65-B set to ready pose")
                else:
                    print("RM65-B using default pose")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str, 
        default='so100',
        choices=['arx-x5', 'so100', 'xarm6_robotiq', 'panda', 'x_fetch', 'unitree_h1', 'piper', 'widowx250s', 'rm65b'],
    ...
    parser.add_argument(
        ...
        type=str,
        default='default',
        choices=['default', 'home', 'standing', 't_pose', 'ready'],
