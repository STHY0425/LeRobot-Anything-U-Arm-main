...
| 发布频率 | 与读取频率相同 (~7Hz串行) | ✅ **独立30Hz**，平滑输出 |

### 双线程工作原理

#### 线程1：舵机读取线程（生产者）
```
频率: ~7Hz (串行读取7个舵机，每个约20ms，总计约140ms)
职责:
  1. 通过串口串行读取7个舵机当前角度（非并行，受限于单串口）
...
> **注意**：舵机读取频率 (~7Hz) 由**串行读取**决定，**不可调**。
> 代码采用串行方式读取7个舵机，每个约20ms，总计约140ms，理论最大频率约 7Hz。
> 
> 如需提高频率，需要：
> - 使用支持并行读取的舵机控制板
> - 或改用总线协议（如RS485/CAN）批量读取
...
6. **双线程架构**：读取 (~7Hz串行) 和发布 (30Hz) 是独立线程，通过队列通信
