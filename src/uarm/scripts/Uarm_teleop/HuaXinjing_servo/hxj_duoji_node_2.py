...
DEFAULTS = {
    ...
    "end_damping_mid": 1100,     # 中档：水平/静止时用，正常手感。
...
class RosPublisher:
    ...
    # 生成某个状态字段的话题数组。
    # 数组长度 = num_servos，用 servo_ids 的位置索引填充，而不是 servo_id 直接做下标。
    @staticmethod
    def make_state_array(shared_state, field_name):
        servo_ids = shared_state.get("servo_ids", list(range(shared_state["num_servos"])))
        data = [0.0] * shared_state["num_servos"]

        for idx, servo_id in enumerate(servo_ids):
            if idx >= len(data):
                break
            servo_state = shared_state["servos"].get(servo_id)
    ...
    def publish_telemetry(telemetry_pubs, shared_state):
        ...
        for idx, sid in enumerate(servo_ids):
            if idx >= num:
                break
            pw_data[idx] = float(shared_state.get("damping_powers", {}).get(sid, 0.0))
...
class Controller:
    ...
    def apply_dynamic_damping(self, manager):
        ...
        #    不在线的舵机（angle=None）默认 0°，不 return 跳过整轮。
        self.state_lock.acquire()
        try:
        ...
        # 跳过不在线的舵机（如 ID=0 不通信的底座），避免向串口发指令给不存在的舵机。
        damping_powers = {}
        for sid in axial_ids:
            damping_powers[sid] = base_pw
            servo_state = self.shared_state["servos"].get(sid, {})
            if not servo_state.get("online", False):
                continue
    ...
    def run(self):
        ...
        try:
            ...
            while not self.stop_event.is_set() and not self.rospy.is_shutdown():
                try:
                    # 逐个读角度，ID=0 手动赋 0 不读串口。
                    new_states = {}
                    for sid in self.config["servo_ids"]:
                        if sid == 0:
                            new_states[0] = {
                                "id": 0, "angle": 0.0, "current": 0.0,
                                "voltage": 0.0, "power": 0.0, "temp": 0.0,
                                "status": 0, "turn": 0.0, "online": False,
                                "stamp": time.time(),
                            }
                        else:
                            angle = self.manager.query_servo_angle(sid)
                            new_states[sid] = {
                                "id": sid, "angle": float(angle) if angle is not None else 0.0,
                                "current": 0.0, "voltage": 0.0, "power": 0.0,
                                "temp": 0.0, "status": 0, "turn": 0.0,
                                "online": angle is not None, "stamp": time.time(),
                            }
...
def main():
    ...
    # 创建配置读取对象。
    # servo_ids 和 num_servos 不传，由 JSON 的 dof 自动派生为 [0, 1, ..., dof-1]。
    # ID=0 是不通信的 axial 底座，控制线程读角度时手动赋 0°，不读串口。
