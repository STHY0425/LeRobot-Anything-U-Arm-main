...
class HxjDuojiNodeTest(unittest.TestCase):
    ...
    def test_shared_state_has_zero_angles(self):
        """shared_state 包含 zero_angles 和 zero_angles_confirmed 字段。"""
        shared_state = node.ServoConfig.create_shared_state_from_values([0, 1, 2], 3)
        self.assertIn("zero_angles", shared_state)
        self.assertIn("zero_angles_confirmed", shared_state)
        self.assertEqual(len(shared_state["zero_angles"]), 3)
        self.assertEqual(shared_state["zero_angles"], [0.0, 0.0, 0.0])
        self.assertFalse(shared_state["zero_angles_confirmed"])

    def test_copy_shared_state_includes_zero_angles(self):
        """copy_shared_state 复制 zero_angles 和 zero_angles_confirmed。"""
        lock = node.threading.Lock()
        shared_state = node.ServoConfig.create_shared_state_from_values([0, 1], 2)
        shared_state["zero_angles"] = [0.0, 0.0]
        shared_state["zero_angles_confirmed"] = True
        copied = node.RosPublisher.copy_shared_state(shared_state, lock)
        self.assertEqual(copied["zero_angles"], [0.0, 0.0])
        self.assertTrue(copied["zero_angles_confirmed"])

    def test_controller_has_reset_zero_flag(self):
        """Controller 有 reset_zero_requested flag，初始为 False。"""
        lock = node.threading.Lock()
        shared_state = node.ServoConfig.create_shared_state_from_values([0], 1)
        stop_event = node.threading.Event()
        worker = node.Controller({}, shared_state, lock, stop_event, None)
        self.assertFalse(worker.reset_zero_requested)

    def test_controller_has_drive_to_zero_method(self):
        """Controller 有 _drive_to_zero 方法。"""
        self.assertTrue(hasattr(node.Controller, "_drive_to_zero"))

    def test_controller_has_confirm_zero_angles_method(self):
        """Controller 有 _confirm_zero_angles 方法。"""
        self.assertTrue(hasattr(node.Controller, "_confirm_zero_angles"))

    def test_controller_has_reset_zero_callback(self):
        """Controller 有 reset_zero_callback 方法。"""
        self.assertTrue(hasattr(node.Controller, "reset_zero_callback"))

    def test_confirm_zero_angles_sets_confirmed(self):
        """_confirm_zero_angles 设 zero_angles_confirmed=True。"""
        lock = node.threading.Lock()
        shared_state = node.ServoConfig.create_shared_state_from_values([0, 1, 2], 3)
        stop_event = node.threading.Event()
        config = {"num_servos": 3}
        worker = node.Controller(config, shared_state, lock, stop_event, None)
        worker._confirm_zero_angles()
        self.assertTrue(shared_state["zero_angles_confirmed"])
        self.assertEqual(shared_state["zero_angles"], [0.0, 0.0, 0.0])

    def test_reset_zero_callback_sets_flag(self):
        """reset_zero_callback 收到 True 时设 reset_zero_requested=True。"""
        lock = node.threading.Lock()
        shared_state = node.ServoConfig.create_shared_state_from_values([0], 1)
        stop_event = node.threading.Event()
        worker = node.Controller({}, shared_state, lock, stop_event, None)
        ...
        worker.reset_zero_callback(FakeBool(True))
        self.assertTrue(worker.reset_zero_requested)

    def test_reset_zero_callback_ignores_false(self):
        """reset_zero_callback 收到 False 时不设 flag。"""
        lock = node.threading.Lock()
        shared_state = node.ServoConfig.create_shared_state_from_values([0], 1)
        stop_event = node.threading.Event()
        worker = node.Controller({}, shared_state, lock, stop_event, None)
        ...
        worker.reset_zero_callback(FakeBool(False))
        self.assertFalse(worker.reset_zero_requested)

    def test_zero_angles_array_size_matches_dof(self):
        """zero_angles 数组长度 = num_servos（JSON dof 动态决定，不写死）。"""
        for dof in [3, 5, 7]:
            shared_state = node.ServoConfig.create_shared_state_from_values(
                list(range(dof)), dof)
            self.assertEqual(len(shared_state["zero_angles"]), dof)
